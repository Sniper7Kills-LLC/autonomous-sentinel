...
export const AbuseReport = a
  ...
  .authorization((allow) => [
    ...
    // pick up `create` via the merged elevated rule below (Amplify
    // @auth rejects the same group in two `allow.groups(...)` rules
    // per model, so the elevated rule absorbs `create` alongside the
    // existing `read` + `update`).
    allow.groups(['member']).to(['create']),
    // Reporter = the Cognito sub stored in `reporterId` (#259).
    allow.ownerDefinedIn('reporterId').identityClaim('sub').to(['read']),
    allow.groups(['moderator', 'admin']).to(['create', 'read', 'update']),
  ]);
