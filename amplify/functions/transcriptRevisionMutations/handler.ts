...
interface Deps {
  ...
  audit?: AuditFn;
  now?: () => Date;
  /** Injectable for tests; defaults to the real reputation recompute (#480). */
...
async function dispatchAccept(
  ...
  // Step 5: recompute the proposer's reputation — an accepted correction
  // is a reputation input (#480). Best-effort: a recompute failure must
  // not fail the accept (the revision is already accepted + transcript
  // rewritten). Inline (not a DDB stream) to avoid the data-stack CFN
  // cycle that streaming TranscriptRevision would close (#658/#661).
  try {
    await deps.repRecompute(deps.client as unknown as ReputationHelperClient, target.proposedBy);
  } catch (err) {
    console.error('acceptTranscriptRevision: reputation recompute failed (non-fatal)', {
      proposedBy: target.proposedBy,
      err,
    });
  }
...
> = async (event, _context, _callback) => {
  ...
  const repRecompute = injected.repRecompute ?? recomputeReputation;
  ...
  switch (field) {
    ...
    case 'acceptTranscriptRevision':
      return dispatchAccept(event, { client, audit: auditFn, now, repRecompute });
    default:
