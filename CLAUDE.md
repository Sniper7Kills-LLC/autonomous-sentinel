...
9. **Resolve every finding from the self-review.** TDD where the fix is a behavior change (failing test → fix → green). For each finding, either fix it in a follow-up commit on the same branch OR open a follow-up issue if it's genuinely out of scope and link it in the reply. Re-run `npm run lint && npm run typecheck && npm run test` locally before pushing. Reply on the **self-review comment thread from step 8** with a short receipt of what was fixed vs deferred (mirrors the pattern used on PR #268 / #269). Multi-commit fixes still get a single rollup receipt on that same thread so the audit trail stays in one place.
...
- **Every PR gets a self-review** — and the self-review must be posted, including the empty case. Spawn `cavecrew-reviewer` against the diff, post findings as a PR review, resolve each finding (fix in a follow-up commit, or open a follow-up issue if genuinely out of scope) before requesting merge. If the reviewer returns no findings, still post an explicit one-line "self-review: no findings" comment so the audit trail is uniform across every PR.
